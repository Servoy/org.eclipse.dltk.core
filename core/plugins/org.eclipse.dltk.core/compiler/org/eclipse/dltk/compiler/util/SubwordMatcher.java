...
public class SubwordMatcher {
	...
	public int[] getMatchingRegions(String pattern) {
		...
		for (int iPattern = 0; iPattern < pattern.length(); iPattern++) {
			...
			if (!isWordBoundary(iName)
					&& equalsIgnoreCase(patternChar, nameChar)) {
				// we're not at a word boundary, case-insensitive match is fine
				continue;
			}

			// not matching, record previous segment and find next word match in
			// name
			...
			if (wordStart < 0) {
				// no matching word found, backtrack and try to find next
				// occurrence of current word
				int next = indexOfWordStart(iName,
						pattern.charAt(iPatternWordStart));
				if (next > 0) {
					wordStart = next;
					iPattern = iPatternWordStart;
					// last recorded segment was invalid -> drop it
					segments = Arrays.copyOfRange(segments, 0,
							segments.length - 2);
				}
			}
	...
	/**
	 * Returns the index of the first word after nameStart, beginning with
	 * patternChar. Returns -1 if no matching word is found.
	 */
	private int indexOfWordStart(int nameStart, char patternChar) {
		...
		for (int iName = nameStart; iName < this.name.length; iName++) {
			char nameChar = this.name[iName];
			if (isWordBoundary(iName)
					&& equalsIgnoreCase(nameChar, patternChar)) {
				return iName;
			}

			// don't match across identifiers (e.g. "index" should not match
			// "substring(int beginIndex)")
			if (!ScannerHelper.isScriptIdentifierPart(nameChar)) {
				return -1;
			}
		}
